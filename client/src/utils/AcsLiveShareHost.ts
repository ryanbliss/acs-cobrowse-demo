...
/**
 * Configuration options for initializing the ACSLiveShareHost class
 */
...
/**
 * Azure Communication Services (ACS) ILiveShareHost implementation, which is intended for use with the LiveShareClient class.
...
export class ACSLiveShareHost implements ILiveShareHost {
    ...
    private get remoteClients(): IClientInfo[] {
        return this.options.call.remoteParticipants.map((participant) => {
            return {
                userId: participant.identifier.kind === "microsoftTeamsUser" ? participant.identifier.microsoftTeamsUserId : "",
    ...
    private constructor(private readonly options: ACSLiveShareHostOptions) {}

    public static create(options: ACSLiveShareHostOptions): ILiveShareHost {
        return new ACSLiveShareHost(options);
    }
    ...
    public async getFluidTenantInfo(): Promise<IFluidTenantInfo> {
        ...
        if (!isIFluidTenantInfoResponse(data)) {
            throw new Error("ACSLiveShareHost.getFluidTenantInfo: invalid response");
        }
    ...
    public async getFluidToken(containerId?: string): Promise<string> {
        ...
        if (!isIFluidTokenResponse(data)) {
            throw new Error("ACSLiveShareHost.getFluidToken: invalid response from server");
        }
        return data.token;
    }
    ...
    public async getFluidContainerId(): Promise<IFluidContainerInfo> {
        ...
        if (!isIFluidContainerInfo(data)) {
            throw new Error("ACSLiveShareHost.IFluidContainerInfo: invalid response from server");
        }
        return data;
    }
    ...
    public async getClientInfo(
        ...
        if (!isIGetClientInfoResponse(data)) {
            throw new Error("ACSLiveShareHost.getClientInfo: invalid response from server");
        }
        return this.clients.find((checkClient) => checkClient.userId === data.userId);
    }
    ...
    private participantRoleToUserMeetingRoles(role: ParticipantRole): UserMeetingRole[] {
        switch (role) {
            case "Co-organizer":
            case "Organizer":
